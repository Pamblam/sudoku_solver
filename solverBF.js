...
class SudokuSolverBruteForce{
	...
	validateSubgrid(x, y, board){
		var numbers = this.getNumbersInSubgrid(x, y, board);
		return [...new Set(numbers)].length === 9;
	}
	validateBoard(board){
		for(var y=0; y<3; y++){
			for(var x=0; x<3; x++){
				if(!this.validateSubgrid(x, y, board)){
					return false;
				}
			}
		}
	...
	checkNext(){
		...
		this.permutation++;
		var board = this.getPermutation(this.permutation);
		var isValid = this.validateBoard(board);
		var str = '';
		for(var y = 0; y<9; y++){
			for(var x=0; x<9; x++){
				str += this.board[y][x] || board[y][x];
				if(this.board[y][x] === null){
					this.renderer.drawNumber(x, y, board[y][x], isValid?'blue':'red');
				}
			}
		}
		return isValid;
